...
export default function App() {
  ...
  const [quizState, setQuizState] = useState({ active: false, idx: 0, answers: [], finished: false });
  const [chat, setChat] = useState([{ role: 'assistant', content: 'Selamat datang di ECONOVA! Saya tutor AI Anda — siap membantu menjelaskan konsep investasi, analisis saham IDX, atau mendampingi Proyek Akhir Anda. Mulai dari mana?' }]);
  ...
  const answerQuiz = (optIdx) => {
    ...
    if (quizState.idx + 1 >= qs.length) {
      ...
      if (score >= Math.floor(qs.length * 0.8)) markDone(`quiz_${activeMod.id}`);
    } else {
  ...
  const sendChat = async () => {
    ...
    try {
      const hist = [...chat, { role: 'user', content: msg }].slice(-12).map((m) => ({ role: m.role, content: m.content }));
      const reply = await callClaude(hist, AI_SYS);
      setChat((p) => [...p, { role: 'assistant', content: reply }]);
    } catch { setChat((p) => [...p, { role: 'assistant', content: '⚠️ Koneksi error. Coba lagi.' }]); }
    setChatBusy(false);
  };
  ...
  const runAI = async () => {
    ...
    try { const r = await callClaude([{ role: 'user', content: aiInput }], AI_SYS); setAiResult(r); }
    catch { setAiResult('Gagal. Coba lagi.'); }
    setAiBusy(false);
  };

  // ── Level Badge ───
  const LevelBadge = ({ level }) => {
    const cfg = { Pemula: { bg: '#dcfce7', c: '#16a34a' }, Menengah: { bg: '#fef9c3', c: '#b45309' }, Lanjutan: { bg: '#fee2e2', c: '#dc2626' } };
    const s = cfg[level] || cfg.Pemula;
    return <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 20, background: s.bg, color: s.c }}>{level}</span>;
  };
    ...
    <>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        html{font-family:'DM Sans',sans-serif;font-size:16px}
        ...
        .fade{animation:fadeUp 0.4s cubic-bezier(0.16,1,0.3,1) both}
        .card-h{transition:all 0.2s ease;cursor:pointer}
        .card-h:hover{transform:translateY(-3px);box-shadow:0 12px 32px rgba(22,163,74,0.1)!important}
        .btn-h{transition:all 0.18s ease}
        .btn-h:hover{opacity:0.92;transform:translateY(-1px)}
        .nav-h:hover{background:${G.primaryLight}!important;color:${G.primary}!important}
        .lesson-r:hover{background:${G.primaryPale}!important;cursor:pointer}
        .opt-b:hover{border-color:${G.primary}!important;background:${G.primaryPale}!important}
        .opt-b{transition:all 0.15s}
        textarea,input{caret-color:${G.primary};outline:none}
        a{text-decoration:none;color:inherit}
        .tag-pill{font-size:10px;font-weight:600;padding:2px 10px;border-radius:20px;background:${G.primaryLight};color:${G.primary};letter-spacing:0.06em;text-transform:uppercase}
      `}</style>

      <div style={{ background: G.bg, color: G.text, minHeight: '100vh', fontFamily: "'DM Sans',sans-serif" }}>

        {/* ── HEADER ─────────────────────────────────────────────────────────── */}
        <header style={{ background: 'rgba(255,255,255,0.96)', borderBottom: `1px solid ${G.border}`, position: 'sticky', top: 0, zIndex: 200, backdropFilter: 'blur(16px)' }}>
          <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 28px', height: 58, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer' }} onClick={() => setPage('home')}>
              <div style={{ width: 34, height: 34, background: G.primary, borderRadius: 9, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
                  <polyline points="22 7 13.5 15.5 8.5 10.5 2 17"/><polyline points="16 7 22 7 22 13"/>
                </svg>
              </div>
              <div>
                <div style={{ fontFamily: "'DM Serif Display',serif", fontWeight: 400, fontSize: 17, color: G.text, lineHeight: 1 }}>ECONOVA</div>
                <div style={{ fontSize: 9, color: G.textMuted, letterSpacing: '0.12em', textTransform: 'uppercase', fontWeight: 500 }}>Econova Advisory</div>
              </div>
            </div>

            {/* Nav */}
            <nav style={{ display: 'flex', gap: 2, alignItems: 'center' }}>
              {[['home','Beranda'],['modul','Modul'],['analisis','Analisis AI'],['chat','Tutor AI'],['kontak','Kontak']].map(([id, lb]) => (
                <button key={id} className="nav-h" onClick={() => setPage(id)} style={{ background: page === id ? G.primaryLight : 'transparent', color: page === id ? G.primary : G.textSoft, border: 'none', borderRadius: 7, padding: '6px 13px', fontSize: 13, fontWeight: page === id ? 600 : 400, cursor: 'pointer' }}>
                  {lb}{id === 'chat' && <span style={{ marginLeft: 5, background: G.primary, color: '#fff', borderRadius: 4, fontSize: 8, padding: '1px 5px', fontWeight: 700 }}>AI</span>}
                </button>
              ))}
            </nav>

            {/* Progress */}
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: G.primaryPale, borderRadius: 20, padding: '5px 14px', border: `1px solid ${G.primaryBorder}` }}>
              <div style={{ width: 40, height: 4, background: G.primaryBorder, borderRadius: 2, overflow: 'hidden' }}>
                <div style={{ width: `${progress}%`, height: '100%', background: G.primary, transition: 'width 0.6s ease', borderRadius: 2 }} />
              </div>
              <span style={{ fontSize: 11, color: G.primary, fontWeight: 600 }}>{progress}%</span>
            </div>
          </div>
        </header>
        ...
        <main style={{ maxWidth: 1200, margin: '0 auto', padding: '32px 28px' }}>
          ...
          {page === 'home' && (
            <div className="fade">
              ...
              {/* Hero */}
              <div style={{ background: G.text, borderRadius: 20, padding: '52px 56px', marginBottom: 24, position: 'relative', overflow: 'hidden' }}>
                <div style={{ position: 'absolute', top: 0, right: 0, width: 480, height: '100%', background: `linear-gradient(135deg, transparent 40%, rgba(22,163,74,0.12))`, pointerEvents: 'none' }} />
                <div style={{ position: 'absolute', bottom: -80, right: 80, width: 200, height: 200, borderRadius: '50%', border: '1px solid rgba(22,163,74,0.15)', pointerEvents: 'none' }} />
                <div style={{ position: 'absolute', bottom: -40, right: 20, width: 300, height: 300, borderRadius: '50%', border: '1px solid rgba(22,163,74,0.08)', pointerEvents: 'none' }} />

                <div style={{ position: 'relative', maxWidth: 580 }}>
                  <div className="tag-pill" style={{ background: 'rgba(22,163,74,0.2)', color: '#4ade80', display: 'inline-block', marginBottom: 22 }}>
                    Platform Edukasi Investasi Saham
                  </div>
                  <h1 style={{ fontFamily: "'DM Serif Display',serif", fontWeight: 400, fontSize: 44, color: '#f8fafc', lineHeight: 1.15, marginBottom: 16 }}>
                    Kuasai Investasi Saham<br/><em style={{ color: '#4ade80' }}>Dari Nol Hingga Mahir</em>
                  </h1>
                  <p style={{ fontSize: 15, color: '#94a3b8', lineHeight: 1.8, marginBottom: 32, fontWeight: 300 }}>
                    8 modul terstruktur · 80 materi lengkap · 105 soal kuis · Tutor AI 24/7.<br/>
                    Analisis fundamental, teknikal, psikologi investasi, dan portofolio nyata.
                  </p>
                  <div style={{ display: 'flex', gap: 12 }}>
                    <button className="btn-h" onClick={() => setPage('modul')} style={{ background: G.primary, color: '#fff', border: 'none', borderRadius: 10, padding: '12px 28px', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>
                      Mulai Belajar →
                    </button>
                    <button className="btn-h" onClick={() => setPage('chat')} style={{ background: 'rgba(255,255,255,0.07)', color: '#e2e8f0', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 10, padding: '12px 24px', fontSize: 14, fontWeight: 400, cursor: 'pointer' }}>
                      Tanya Tutor AI
                    </button>
                  </div>
                </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 14, marginBottom: 32 }}>
                {[
                  { val: '8', label: 'Modul Lengkap', sub: 'Dari dasar ke mahir' },
                  { val: '80', label: 'Total Materi', sub: 'Konten mendalam' },
                  { val: '105', label: 'Soal Kuis', sub: 'Uji pemahaman' },
                  { val: 'AI', label: 'Tutor Interaktif', sub: 'Tersedia 24/7' },
                ].map((s, i) => (
                  <div key={i} className="card-h" onClick={() => setPage(i < 3 ? 'modul' : 'chat')}
                    style={{ background: G.white, border: `1px solid ${G.border}`, borderRadius: 14, padding: '20px 22px', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
                    <div style={{ fontFamily: "'DM Serif Display',serif", fontWeight: 400, fontSize: 32, color: G.primary, lineHeight: 1 }}>{s.val}</div>
                    <div style={{ fontSize: 13, fontWeight: 600, color: G.text, marginTop: 6 }}>{s.label}</div>
                    <div style={{ fontSize: 11, color: G.textMuted, marginTop: 2 }}>{s.sub}</div>
                  </div>
                ))}
              </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                <h2 style={{ fontFamily: "'DM Serif Display',serif", fontWeight: 400, fontSize: 22, color: G.text }}>Kurikulum 8 Modul</h2>
                <button onClick={() => setPage('modul')} style={{ background: 'none', border: `1px solid ${G.border}`, borderRadius: 8, padding: '6px 14px', fontSize: 12, color: G.textSoft, cursor: 'pointer', fontFamily: "'DM Sans',sans-serif" }}>
                  Lihat Semua →
                </button>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 14, marginBottom: 32 }}>
                {MODULES.map((mod) => {
                  ...
                  return (
                    <div key={mod.id} className="card-h" style={{ background: G.white, border: `1px solid ${G.border}`, borderRadius: 14, padding: 20, boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }} onClick={() => setPage('modul')}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                          <div style={{ width: 40, height: 40, background: G.primaryPale, borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}>{mod.icon}</div>
                          <div>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3 }}>
                              <span style={{ fontSize: 10, fontWeight: 700, color: G.textMuted, letterSpacing: '0.08em', textTransform: 'uppercase' }}>M{mod.num}</span>
                              <LevelBadge level={mod.level} />
                            </div>
                            <div style={{ fontSize: 14, fontWeight: 600, color: G.text }}>{mod.title}</div>
                          </div>
                        </div>
                        <span style={{ fontFamily: "'DM Serif Display',serif", fontSize: 16, color: G.primary }}>{done}/{mod.lessons.length}</span>
                      </div>
                      <p style={{ fontSize: 11, color: G.textMuted, lineHeight: 1.6, marginBottom: 10 }}>{mod.desc}</p>
                      <div style={{ height: 3, background: G.primaryBorder, borderRadius: 2, overflow: 'hidden' }}>
                        <div style={{ width: `${(done / mod.lessons.length) * 100}%`, height: '100%', background: G.primary, borderRadius: 2, transition: 'width 0.6s ease' }} />
                      </div>
                    </div>
                  );
                })}
              ...
              {/* AI Features */}
              <div style={{ background: G.white, border: `1px solid ${G.border}`, borderRadius: 16, padding: 28, boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
                  <div style={{ width: 36, height: 36, background: G.primary, borderRadius: 9, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16 }}>🤖</div>
                  <div>
                    <div style={{ fontFamily: "'DM Serif Display',serif", fontSize: 18, color: G.text }}>Fitur AI Tutor ECONOVA</div>
                    <div style={{ fontSize: 12, color: G.textMuted }}>Tanyakan apa saja — tersedia 24/7</div>
                  </div>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12 }}>
                  {[
                    { icon: '🔍', title: 'Analisis Saham AI', desc: 'Analisis fundamental & teknikal saham IDX apa saja', page: 'analisis' },
                    { icon: '💬', title: 'Chat Tutor', desc: 'Diskusi bebas soal investasi, minta penjelasan ulang', page: 'chat' },
                    { icon: '🏆', title: 'Proyek Akhir', desc: 'Dampingi AI untuk selesaikan simulasi portofolio', page: 'modul' },
                  ].map((f, i) => (
                    <div key={i} className="card-h" style={{ background: G.primaryPale, border: `1px solid ${G.primaryBorder}`, borderRadius: 12, padding: 18 }} onClick={() => setPage(f.page)}>
                      <div style={{ fontSize: 22, marginBottom: 8 }}>{f.icon}</div>
                      <div style={{ fontWeight: 600, fontSize: 13, color: G.text, marginBottom: 4 }}>{f.title}</div>
                      <div style={{ fontSize: 11, color: G.textSoft, lineHeight: 1.6 }}>{f.desc}</div>
                    </div>
                  ))}
                </div>
              </div>
          ...
          {page === 'modul' && (
            <div className="fade">
              <div style={{ marginBottom: 24 }}>
                <h1 style={{ fontFamily: "'DM Serif Display',serif", fontSize: 28, color: G.text, marginBottom: 6 }}>Modul Pembelajaran</h1>
                <p style={{ fontSize: 13, color: G.textSoft }}>Ikuti setiap modul secara berurutan untuk hasil pembelajaran terbaik.</p>
              </div>
              ...
              {MODULES.map((mod) => {
                ...
                return (
                  <div key={mod.id} style={{ background: G.white, border: `1px solid ${G.border}`, borderRadius: 16, overflow: 'hidden', marginBottom: 16, boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
                    {/* Module Header */}
                    <div style={{ padding: '20px 24px', borderBottom: `1px solid ${G.border}` }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                        <div style={{ width: 48, height: 48, background: G.primaryPale, borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22 }}>{mod.icon}</div>
                        <div style={{ flex: 1 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                            <span style={{ fontSize: 10, fontWeight: 700, color: G.primary, letterSpacing: '0.1em', textTransform: 'uppercase' }}>MODUL {mod.num}</span>
                            <LevelBadge level={mod.level} />
                            {quizDone && <span style={{ fontSize: 10, fontWeight: 600, color: G.primary, background: G.primaryLight, padding: '2px 8px', borderRadius: 8 }}>✓ Kuis Selesai</span>}
                          </div>
                          <h2 style={{ fontFamily: "'DM Serif Display',serif", fontSize: 17, color: G.text, marginBottom: 2 }}>{mod.title}</h2>
                          <p style={{ fontSize: 11, color: G.textMuted }}>{mod.desc}</p>
                        </div>
                        <div style={{ textAlign: 'right' }}>
                          <div style={{ fontFamily: "'DM Serif Display',serif", fontSize: 20, color: G.primary }}>{done}/{mod.lessons.length}</div>
                          <div style={{ fontSize: 10, color: G.textMuted }}>selesai</div>
                        </div>
                      </div>
                      <div style={{ marginTop: 12, height: 3, background: G.primaryBorder, borderRadius: 2, overflow: 'hidden' }}>
                        <div style={{ width: `${(done / mod.lessons.length) * 100}%`, height: '100%', background: G.primary, borderRadius: 2, transition: 'width 0.6s ease' }} />
                      </div>
                    </div>
                    ...
                    {mod.lessons.map((les, i) => {
                      ...
                      return (
                        <div key={les.id} className="lesson-r" style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 24px', borderBottom: i < mod.lessons.length - 1 ? `1px solid ${G.bg}` : 'none', transition: 'background 0.15s' }} onClick={() => openLesson(mod, les)}>
                          <div style={{ width: 30, height: 30, borderRadius: 8, flexShrink: 0, background: isDone ? G.primaryLight : G.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, color: isDone ? G.primary : G.textMuted }}>
                            {isDone ? '✓' : i + 1}
                          </div>
                          <div style={{ flex: 1 }}>
                            <div style={{ fontSize: 13, fontWeight: 500, color: G.text, marginBottom: 1 }}>{les.title}</div>
                            <div style={{ fontSize: 11, color: G.textMuted }}>⏱ {les.dur}</div>
                          </div>
                          {isDone && <span style={{ fontSize: 10, fontWeight: 600, color: G.primary, background: G.primaryLight, padding: '2px 8px', borderRadius: 7 }}>Selesai</span>}
                          <span style={{ fontSize: 14, color: G.primaryBorder }}>›</span>
                        </div>
                      );
                    })}
                    ...
                    {mod.quiz && mod.quiz.length > 0 && (
                      <div style={{ padding: '14px 24px', background: G.primaryPale, borderTop: `1px solid ${G.primaryBorder}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
                        <div>
                          <div style={{ fontSize: 12, fontWeight: 600, color: G.primary }}>Kuis Modul · {mod.quiz.length} Soal</div>
                          <div style={{ fontSize: 11, color: G.textSoft }}>Selesaikan semua materi lalu uji pemahaman Anda</div>
                        </div>
                        <button className="btn-h" onClick={() => { setActiveMod(mod); setActiveLes(null); setPage('quiz'); setQuizState({ active: true, idx: 0, answers: [], finished: false }); }}
                          style={{ background: G.primary, color: '#fff', border: 'none', borderRadius: 8, padding: '8px 18px', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
          ...
          {page === 'lesson' && activeLes && (
            <div className="fade" style={{ maxWidth: 780, margin: '0 auto' }}>
              {/* Breadcrumb */}
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: G.textMuted, marginBottom: 20 }}>
                <span style={{ cursor: 'pointer', color: G.textSoft }} onClick={() => setPage('modul')}>Modul</span>
                <span>›</span>
                <span style={{ color: G.primary, fontWeight: 500 }}>{activeMod.title}</span>
                <span>›</span>
                <span style={{ color: G.text }}>{activeLes.title}</span>
              </div>

              <div style={{ background: G.white, border: `1px solid ${G.border}`, borderRadius: 18, overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}>
                {/* Header */}
                <div style={{ padding: '24px 32px', borderBottom: `1px solid ${G.border}`, background: G.primaryPale }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                    <LevelBadge level={activeMod.level} />
                    <span style={{ fontSize: 11, color: G.textMuted }}>⏱ {activeLes.dur}</span>
                    {completed.has(activeLes.id) && <span style={{ fontSize: 11, fontWeight: 600, color: G.primary, background: G.primaryLight, padding: '2px 9px', borderRadius: 8 }}>✓ Selesai</span>}
                  </div>
                  <h1 style={{ fontFamily: "'DM Serif Display',serif", fontSize: 22, color: G.text }}>{activeLes.title}</h1>
                </div>
                ...
                {/* Actions */}
                <div style={{ padding: '16px 32px', borderTop: `1px solid ${G.border}`, display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap', background: G.bg }}>
                  {!completed.has(activeLes.id) && (
                    <button className="btn-h" onClick={() => markDone(activeLes.id)} style={{ background: G.primary, color: '#fff', border: 'none', borderRadius: 9, padding: '10px 22px', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
                      ✓ Tandai Selesai
                    </button>
                  )}
                  <button className="btn-h" onClick={() => { setChatIn(`Jelaskan lebih lanjut materi "${activeLes.title}"`); setPage('chat'); }}
                    style={{ background: G.primaryPale, color: G.primary, border: `1px solid ${G.primaryBorder}`, borderRadius: 9, padding: '10px 18px', fontSize: 13, fontWeight: 500, cursor: 'pointer' }}>
                    Tanya AI tentang ini
                  </button>
                  <button onClick={() => setPage('modul')} style={{ background: 'none', color: G.textMuted, border: 'none', padding: '10px 14px', fontSize: 13, cursor: 'pointer', marginLeft: 'auto', fontFamily: "'DM Sans',sans-serif" }}>
                    ← Kembali
                  </button>
                </div>
              </div>
              ...
              {(() => {
                ...
                return next ? (
                  <div className="card-h" style={{ background: G.white, border: `1px solid ${G.border}`, borderRadius: 12, padding: '14px 20px', marginTop: 14, display: 'flex', alignItems: 'center', gap: 12, boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }} onClick={() => openLesson(activeMod, next)}>
                    <div style={{ width: 32, height: 32, background: G.primaryLight, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 12, color: G.primary }}>{idx + 2}</div>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 10, color: G.textMuted }}>Materi Berikutnya</div>
                      <div style={{ fontWeight: 500, fontSize: 14, color: G.text }}>{next.title}</div>
                    </div>
                    <span style={{ fontSize: 16, color: G.primary }}>→</span>
                  </div>
                ) : null;
              })()}
            </div>
          ...
          {page === 'quiz' && activeMod && activeMod.quiz.length > 0 && (
            <div className="fade" style={{ maxWidth: 680, margin: '0 auto' }}>
              <div style={{ background: G.white, border: `1px solid ${G.border}`, borderRadius: 18, overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}>
                {!quizState.finished ? (
                  <>
                    <div style={{ padding: '20px 28px', borderBottom: `1px solid ${G.border}`, background: G.primaryPale }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                        <div style={{ fontSize: 11, fontWeight: 700, color: G.primary, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Kuis · {activeMod.title}</div>
                        <div style={{ fontSize: 11, color: G.textMuted }}>{Math.round((quizState.idx / activeMod.quiz.length) * 100)}% selesai</div>
                      </div>
                      <div style={{ fontSize: 13, fontWeight: 500, color: G.textSoft, marginBottom: 10 }}>Soal {quizState.idx + 1} dari {activeMod.quiz.length}</div>
                      <div style={{ height: 3, background: G.primaryBorder, borderRadius: 2, overflow: 'hidden' }}>
                        <div style={{ width: `${(quizState.idx / activeMod.quiz.length) * 100}%`, height: '100%', background: G.primary, borderRadius: 2, transition: 'width 0.4s ease' }} />
                      </div>
                    </div>
                    <div style={{ padding: '28px' }}>
                      <div style={{ fontSize: 16, fontWeight: 500, color: G.text, lineHeight: 1.65, marginBottom: 20 }}>{activeMod.quiz[quizState.idx].q}</div>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
                        {activeMod.quiz[quizState.idx].opts.map((opt, i) => (
                          <button key={i} className="opt-b" onClick={() => answerQuiz(i)}
                            style={{ textAlign: 'left', background: G.bg, border: `1.5px solid ${G.border}`, borderRadius: 10, padding: '12px 16px', fontSize: 13, color: G.textSoft, fontFamily: "'DM Sans',sans-serif", fontWeight: 400, cursor: 'pointer' }}>
                            <span style={{ fontWeight: 700, color: G.primary, marginRight: 10 }}>{String.fromCharCode(65 + i)}.</span>{opt}
                          </button>
                        ))}
                      </div>
                    </div>
                  ...
                  <div style={{ padding: '48px 32px', textAlign: 'center' }}>
                    <div style={{ fontSize: 44, marginBottom: 12 }}>
                      {quizState.answers.filter((a) => a.correct).length >= Math.floor(activeMod.quiz.length * 0.8) ? '🎉' : '💪'}
                    </div>
                    <div style={{ fontFamily: "'DM Serif Display',serif", fontSize: 26, color: quizState.answers.filter((a) => a.correct).length >= Math.floor(activeMod.quiz.length * 0.8) ? G.primary : '#dc2626', marginBottom: 8 }}>
                      Skor: {quizState.answers.filter((a) => a.correct).length}/{activeMod.quiz.length} ({Math.round((quizState.answers.filter((a) => a.correct).length / activeMod.quiz.length) * 100)}%)
                    </div>
                    <div style={{ fontSize: 13, color: G.textSoft, marginBottom: 28 }}>
                      {quizState.answers.filter((a) => a.correct).length >= Math.floor(activeMod.quiz.length * 0.8) ? 'Luar biasa! Anda menguasai materi ini dengan baik.' : 'Baca ulang materi lalu coba lagi.'}
                    </div>
                    <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap' }}>
                      <button className="btn-h" onClick={() => setQuizState({ active: true, idx: 0, answers: [], finished: false })}
                        style={{ background: G.primaryPale, color: G.primary, border: `1px solid ${G.primaryBorder}`, borderRadius: 9, padding: '10px 20px', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
                        Ulangi Kuis
                      </button>
                      <button className="btn-h" onClick={() => setPage('modul')}
                        style={{ background: G.primary, color: '#fff', border: 'none', borderRadius: 9, padding: '10px 22px', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
                        Lanjut Modul →
                      </button>
                    </div>
                  </div>
          ...
          {page === 'analisis' && (
            <div className="fade" style={{ maxWidth: 820, margin: '0 auto' }}>
              <div style={{ marginBottom: 24 }}>
                <h1 style={{ fontFamily: "'DM Serif Display',serif", fontSize: 28, color: G.text, marginBottom: 6 }}>Analisis Saham dengan AI</h1>
                <p style={{ fontSize: 13, color: G.textSoft }}>Masukkan nama/kode saham atau pertanyaan analisis — AI memberikan penjelasan edukatif lengkap.</p>
              </div>

              <div style={{ background: G.white, border: `1px solid ${G.border}`, borderRadius: 16, padding: 24, marginBottom: 18, boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
                <textarea value={aiInput} onChange={(e) => setAiInput(e.target.value)}
                  placeholder="Contoh: Analisis fundamental BBCA | Jelaskan cara membaca P/E ratio | Strategi DCA untuk pemula?"
                  style={{ width: '100%', background: G.bg, border: `1.5px solid ${G.border}`, borderRadius: 10, padding: '12px 14px', fontSize: 13, color: G.text, resize: 'none', lineHeight: 1.65, fontFamily: "'DM Sans',sans-serif", minHeight: 80, marginBottom: 14 }} />

                <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap', marginBottom: 16 }}>
                  {['Analisis fundamental BBCA','Cara membaca P/E ratio','Apa itu value investing?','Jelaskan DCF dengan contoh','Strategi DCA untuk pemula','Analisis saham GOTO'].map((q, i) => (
                    <button key={i} onClick={() => setAiInput(q)}
                      style={{ background: aiInput === q ? G.primaryLight : G.bg, color: aiInput === q ? G.primary : G.textSoft, border: `1px solid ${aiInput === q ? G.primaryBorder : G.border}`, borderRadius: 16, padding: '4px 12px', fontSize: 11, cursor: 'pointer', fontFamily: "'DM Sans',sans-serif", fontWeight: 500, transition: 'all 0.15s' }}>
                      {q}
                    </button>
                  ))}
                </div>

                <button className="btn-h" onClick={runAI} disabled={aiBusy || !aiInput.trim()}
                  style={{ background: aiInput.trim() && !aiBusy ? G.primary : G.border, color: aiInput.trim() && !aiBusy ? '#fff' : G.textMuted, border: 'none', borderRadius: 9, padding: '11px 26px', fontSize: 13, fontWeight: 600, cursor: aiInput.trim() && !aiBusy ? 'pointer' : 'default', transition: 'all 0.2s' }}>
              ...
              {(aiBusy || aiResult) && (
                <div style={{ background: G.white, border: `1px solid ${G.border}`, borderRadius: 16, padding: 28, boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16, paddingBottom: 14, borderBottom: `1px solid ${G.border}` }}>
                    <div style={{ width: 36, height: 36, background: G.primary, borderRadius: 9, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16 }}>🤖</div>
                    <div>
                      <div style={{ fontWeight: 600, fontSize: 13, color: G.text }}>Tutor AI ECONOVA</div>
                      <div style={{ fontSize: 11, color: G.textMuted }}>Analisis Edukatif Saham IDX</div>
                    </div>
                    {aiBusy && <div style={{ marginLeft: 'auto', width: 18, height: 18, border: `2px solid ${G.primaryBorder}`, borderTopColor: G.primary, borderRadius: '50%', animation: 'spin 0.7s linear infinite' }} />}
                  </div>
                  {aiBusy ? (
                    <div style={{ display: 'flex', gap: 5, padding: '12px 0', alignItems: 'center' }}>
                      {[0,1,2].map((i) => <div key={i} style={{ width: 7, height: 7, borderRadius: '50%', background: G.primary, animation: `dot 1.2s ${i*0.18}s infinite` }} />)}
                      <span style={{ marginLeft: 10, color: G.textMuted, fontSize: 13 }}>AI sedang menganalisis...</span>
                    </div>
                  ) : <div style={{ lineHeight: 1.85 }}>{renderContent(aiResult)}</div>}
                </div>
              )}

              {!aiResult && !aiBusy && (
                <div style={{ background: G.white, border: `1.5px dashed ${G.primaryBorder}`, borderRadius: 16, padding: 48, textAlign: 'center' }}>
                  <div style={{ fontSize: 40, marginBottom: 12 }}>🔍</div>
                  <div style={{ fontFamily: "'DM Serif Display',serif", fontSize: 18, color: G.text, marginBottom: 8 }}>Tanyakan Apa Saja tentang Saham IDX</div>
                  <div style={{ fontSize: 13, color: G.textMuted, maxWidth: 380, margin: '0 auto' }}>Analisis fundamental, teknikal, valuasi, strategi portofolio, atau konsep investasi — AI siap membantu.</div>
                </div>
              )}
            </div>
          )}
          ...
          {page === 'chat' && (
            <div className="fade" style={{ maxWidth: 820, margin: '0 auto' }}>
              <div style={{ marginBottom: 20 }}>
                <h1 style={{ fontFamily: "'DM Serif Display',serif", fontSize: 28, color: G.text, marginBottom: 6 }}>Tutor AI</h1>
                <p style={{ fontSize: 13, color: G.textSoft }}>Diskusi langsung seperti punya mentor investasi pribadi.</p>
              </div>

              <div style={{ display: 'grid', gridTemplateColumns: '1fr 240px', gap: 16, alignItems: 'start' }}>
                {/* Chat Window */}
                <div style={{ background: G.white, border: `1px solid ${G.border}`, borderRadius: 16, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
                  <div style={{ padding: '14px 18px', borderBottom: `1px solid ${G.border}`, display: 'flex', alignItems: 'center', gap: 10, background: G.primaryPale }}>
                    <div style={{ width: 38, height: 38, background: G.primary, borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}>🤖</div>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontWeight: 600, fontSize: 14, color: G.text }}>Tutor AI ECONOVA</div>
                      <div style={{ fontSize: 11, color: G.textMuted }}>Spesialis Edukasi Saham Indonesia</div>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 11, color: G.primary }}>
                      <span style={{ width: 6, height: 6, background: G.primary, borderRadius: '50%', animation: 'pulse 2s infinite', display: 'inline-block' }} />
                      Online
                    </div>
                  </div>

                  <div ref={chatRef} style={{ height: 420, overflowY: 'auto', padding: 16, display: 'flex', flexDirection: 'column', gap: 10 }}>
                    {chat.map((m, i) => (
                      <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: m.role === 'user' ? 'flex-end' : 'flex-start', gap: 3 }}>
                        {m.role === 'assistant' && <div style={{ fontSize: 10, color: G.textMuted, paddingLeft: 2 }}>Tutor AI</div>}
                        <div style={{ maxWidth: '86%', background: m.role === 'user' ? G.primary : G.primaryPale, border: m.role === 'user' ? 'none' : `1px solid ${G.primaryBorder}`, color: m.role === 'user' ? '#fff' : G.textSoft, borderRadius: m.role === 'user' ? '16px 16px 4px 16px' : '16px 16px 16px 4px', padding: '10px 14px', fontSize: 13, lineHeight: 1.75, whiteSpace: 'pre-wrap' }}>
                          {m.role === 'assistant' ? renderContent(m.content) : m.content}
                        </div>
                      </div>
                    ))}
                    {chatBusy && (
                      <div style={{ alignSelf: 'flex-start', background: G.primaryPale, border: `1px solid ${G.primaryBorder}`, borderRadius: '16px 16px 16px 4px', padding: '11px 14px', display: 'flex', gap: 4, alignItems: 'center' }}>
                        {[0,1,2].map((i) => <div key={i} style={{ width: 6, height: 6, borderRadius: '50%', background: G.primary, animation: `dot 1.2s ${i*0.18}s infinite` }} />)}
                        <span style={{ marginLeft: 8, color: G.textMuted, fontSize: 12 }}>AI sedang mengetik...</span>
                      </div>
                    )}
                  </div>

                  <div style={{ padding: '12px 16px', borderTop: `1px solid ${G.border}`, display: 'flex', gap: 8, background: G.bg }}>
                    <textarea value={chatIn} onChange={(e) => setChatIn(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendChat(); } }}
                      placeholder="Tanya tentang investasi saham..." rows={2}
                      style={{ flex: 1, background: G.white, border: `1px solid ${G.border}`, borderRadius: 9, padding: '9px 12px', fontSize: 13, color: G.text, resize: 'none', lineHeight: 1.5, fontFamily: "'DM Sans',sans-serif" }} />
                    <button onClick={sendChat} disabled={chatBusy || !chatIn.trim()}
                      style={{ background: chatIn.trim() && !chatBusy ? G.primary : G.border, color: chatIn.trim() && !chatBusy ? '#fff' : G.textMuted, border: 'none', borderRadius: 9, padding: '0 18px', fontSize: 13, fontWeight: 600, cursor: chatIn.trim() && !chatBusy ? 'pointer' : 'default', transition: 'all 0.2s', minWidth: 64 }}>
                      {chatBusy ? '...' : 'Kirim'}
                    </button>
                  </div>
                </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  <div style={{ background: G.white, border: `1px solid ${G.border}`, borderRadius: 14, padding: 18, boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
                    <div style={{ fontSize: 11, fontWeight: 700, color: G.textMuted, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 10 }}>Pertanyaan Populer</div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                      {['Apa itu P/E ratio?','Bagaimana cara DCA?','Jelaskan economic moat','Analisis BBCA vs BBRI','Apa itu margin of safety?','Cara membaca laporan keuangan'].map((q, i) => (
                        <button key={i} onClick={() => { setChatIn(q); }}
                          style={{ textAlign: 'left', background: G.primaryPale, border: `1px solid ${G.primaryBorder}`, borderRadius: 7, padding: '7px 10px', fontSize: 11, color: G.primary, cursor: 'pointer', fontFamily: "'DM Sans',sans-serif", transition: 'all 0.15s' }}>
                          {q}
                        </button>
                      ))}
                    </div>
                  </div>
                  <div style={{ background: G.primaryPale, border: `1px solid ${G.primaryBorder}`, borderRadius: 12, padding: 16 }}>
                    <div style={{ fontSize: 11, fontWeight: 700, color: G.primary, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 6 }}>Tips Belajar</div>
                    <div style={{ fontSize: 11, color: G.textSoft, lineHeight: 1.7 }}>Setelah membaca modul, langsung tanyakan ke AI untuk contoh nyata dari saham IDX. Pembelajaran aktif jauh lebih efektif.</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          {page === 'kontak' && (
            <div className="fade" style={{ maxWidth: 720, margin: '0 auto' }}>
              <div style={{ background: G.white, border: `1px solid ${G.border}`, borderRadius: 18, overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}>
                {/* Hero */}
                <div style={{ background: G.text, padding: '44px 40px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
                  <div style={{ position: 'absolute', top: 0, right: 0, width: 300, height: '100%', background: 'linear-gradient(135deg,transparent,rgba(22,163,74,0.1))', pointerEvents: 'none' }} />
                  <div style={{ width: 72, height: 72, background: G.primary, borderRadius: 18, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 32, margin: '0 auto 18px' }}>👤</div>
                  <div style={{ fontFamily: "'DM Serif Display',serif", fontSize: 26, color: '#f8fafc', marginBottom: 5 }}>Septarius Pius</div>
                  <div style={{ fontSize: 13, color: '#94a3b8', marginBottom: 16 }}>Founder & Chief Learning Officer — ECONOVA by Septarius Education</div>
                  <div style={{ display: 'inline-flex', alignItems: 'center', gap: 7, background: 'rgba(22,163,74,0.15)', border: '1px solid rgba(22,163,74,0.3)', borderRadius: 20, padding: '5px 14px' }}>
                    <span style={{ width: 6, height: 6, background: '#4ade80', borderRadius: '50%', animation: 'pulse 2s infinite', display: 'inline-block' }} />
                    <span style={{ fontSize: 11, color: '#86efac', fontWeight: 500 }}>Tersedia untuk konsultasi & kolaborasi</span>
                  </div>
                </div>

                {/* Bio */}
                <div style={{ padding: '28px 40px', borderBottom: `1px solid ${G.border}` }}>
                  <h3 style={{ fontFamily: "'DM Serif Display',serif", fontSize: 18, color: G.text, marginBottom: 10 }}>Tentang ECONOVA</h3>
                  <p style={{ fontSize: 13, color: G.textSoft, lineHeight: 1.85, marginBottom: 10 }}>ECONOVA adalah platform edukasi investasi saham Indonesia yang dibangun dengan misi sederhana: membuat literasi keuangan dan investasi saham bisa diakses oleh semua orang, dari pemula hingga yang ingin memperdalam pengetahuan.</p>
                  <p style={{ fontSize: 13, color: G.textSoft, lineHeight: 1.85 }}>Platform ini menggabungkan kurikulum terstruktur (8 modul · 80 materi · 105 kuis) dengan teknologi AI untuk menciptakan pengalaman belajar yang personal, interaktif, dan efektif.</p>
                </div>

                {/* Contact Cards */}
                <div style={{ padding: '28px 40px' }}>
                  <h3 style={{ fontFamily: "'DM Serif Display',serif", fontSize: 18, color: G.text, marginBottom: 18 }}>Hubungi Saya</h3>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 24 }}>
                    {[
                      { icon: '✉️', label: 'Email', value: 'septariuspius9@gmail.com', desc: 'Untuk pertanyaan, feedback, kolaborasi, atau konsultasi', color: G.primary, bg: G.primaryPale, border: G.primaryBorder, href: 'mailto:septariuspius9@gmail.com' },
                      { icon: '🔗', label: 'LinkedIn', value: 'linkedin.com/in/septariuspius', desc: 'Terhubung secara profesional dan ikuti update terbaru', color: '#0077b5', bg: '#f0f8ff', border: '#bae6fd', href: 'https://www.linkedin.com/in/septariuspius' },
                    ].map((c, i) => (
                      <a key={i} href={c.href} target="_blank" rel="noopener noreferrer" style={{ display: 'block' }}>
                        <div className="card-h" style={{ background: c.bg, border: `1px solid ${c.border}`, borderRadius: 14, padding: 20 }}>
                          <div style={{ fontSize: 24, marginBottom: 10 }}>{c.icon}</div>
                          <div style={{ fontSize: 10, fontWeight: 700, color: c.color, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 3 }}>{c.label}</div>
                          <div style={{ fontWeight: 600, fontSize: 12, color: G.text, marginBottom: 5, wordBreak: 'break-all' }}>{c.value}</div>
                          <div style={{ fontSize: 11, color: G.textSoft, lineHeight: 1.5 }}>{c.desc}</div>
                        </div>
                      </a>
                    ))}
                  </div>

                  <div style={{ background: G.primaryPale, border: `1px solid ${G.primaryBorder}`, borderRadius: 14, padding: 22 }}>
                    <h4 style={{ fontFamily: "'DM Serif Display',serif", fontSize: 15, color: G.text, marginBottom: 12 }}>Apa yang bisa saya bantu?</h4>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                      {['Pertanyaan seputar materi investasi','Feedback dan saran pengembangan platform','Kolaborasi konten edukasi keuangan','Konsultasi strategi portofolio','Diskusi analisis saham IDX','Peluang kerjasama dan partnership'].map((item, i) => (
                        <div key={i} style={{ display: 'flex', gap: 7, alignItems: 'flex-start', fontSize: 12, color: G.textSoft }}>
                          <span style={{ color: G.primary, fontWeight: 700, marginTop: 1, flexShrink: 0 }}>✓</span>
                          <span>{item}</span>
                        </div>
                      ))}
                    </div>
                ...
                <div style={{ padding: '16px 40px', background: G.bg, borderTop: `1px solid ${G.border}`, textAlign: 'center' }}>
                  <div style={{ fontSize: 11, color: G.textMuted }}>⚠️ Seluruh konten ECONOVA bersifat edukatif — bukan saran investasi resmi. Selalu lakukan riset mandiri (DYOR).</div>
                </div>
              </div>
            </div>
          )}
        ...
        {/* ── FOOTER ─────────────────────────────────────────────────────────── */}
        <footer style={{ borderTop: `1px solid ${G.border}`, padding: '14px 28px', background: G.white, marginTop: 40 }}>
          <div style={{ maxWidth: 1200, margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 10 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <span style={{ fontFamily: "'DM Serif Display',serif", fontSize: 14, color: G.text }}>ECONOVA</span>
              <span style={{ fontSize: 11, color: G.textMuted }}>by Econova Advisory</span>
            </div>
            <div style={{ display: 'flex', gap: 14, fontSize: 11, color: G.textMuted }}>
              <span>8 Modul · 80 Materi · 105 Kuis</span>
              <span>·</span>
              <a href="mailto:septariuspius9@gmail.com" style={{ color: G.primary, fontWeight: 600 }}>septariuspius9@gmail.com</a>
              <span>·</span>
              <a href="https://www.linkedin.com/in/septariuspius" target="_blank" rel="noopener noreferrer" style={{ color: '#0077b5', fontWeight: 600 }}>LinkedIn</a>
            </div>
            <div style={{ fontSize: 10, color: G.textMuted }}>⚠️ Edukatif saja — bukan saran investasi resmi. DYOR.</div>
          </div>
        </footer>
